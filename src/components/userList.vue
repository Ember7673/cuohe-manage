 ...
 * @LastEditTime: 2020-12-14 00:00:21
 * @FilePath: \cuohe-manage\src\components\userList.vue
-->
<template>
  <div class="list">
    <el-table ref="singleTable" highlight-current-row @cell-mouse-enter="inviteCodeHover"
      @row-click="handleCurrentChange" :data="list" stripe style="width: 100%">
      ...
      <el-table-column label="操作">
        <template slot-scope="scope">
          <el-button
            v-show="scope.row.requirement_status === '已完善信息，未审核' || scope.row.requirement_status === '修改资料，未审核'"
            type="primary" size="mini" @click.stop="handelExamine(scope.$index, scope.row)">审核</el-button>
        </template>
      </el-table-column>
    </el-table>
  ...
  export default {
    ...
    methods: {
      ...
      inviteCodeHover(row, column, cell) {
        if (column.id !== "el-table_1_column_5") return;
        getUserInfoFromInviteCodeMoudle({
          reqid: uuid(),
